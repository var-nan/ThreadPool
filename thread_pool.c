...
int thread_pool_init_list(thread_pool_t **thr_pool, int num_threads, int max_queue_size, task_t *task_list) {
    ...
    init_tpool_pq(&tpool->task_queue, max_queue_size, task_list);

    // initialize field members and threads
    tpool->num_threads = num_threads;
    tpool->cur_queue_size = task_list == NULL ? 0 : max_queue_size;
...
int thread_pool_init_qs(thread_pool_t **tpool, int num_threads, int max_queue_size) {
    return thread_pool_init_list(tpool, num_threads, max_queue_size, NULL);
}
